...
export const deleteKey = async (req, res) => {
    try {
        const { id } = req.params;
        console.log(`🗑️ Attempting to delete key ID: ${id} for User: ${req.userId}`);
        
        const result = await ApiKey.findOneAndDelete({ _id: id, userId: req.userId });
        if (!result) return res.status(404).json({ error: "Key not found or not yours" });
        
        await User.findByIdAndUpdate(req.userId, { $pull: { apiKeys: id } });
        res.json({ success: true });
    } catch (err) {
        console.error("🔥 Delete Key Error:", err); // 👈 ADD THIS
...
export const toggleKey = async (req, res) => {
    try {
        const { id } = req.params;
        console.log(`🔄 Attempting to toggle key ID: ${id} for User: ${req.userId}`);
        ...
        console.error("🔥 Toggle Key Error:", err); // 👈 ADD THIS
