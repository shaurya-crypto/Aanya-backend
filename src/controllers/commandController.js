...
export async function handleCommand(req, res) {
  try {
    ...
    **AVAILABLE INTENT TYPES:**
    ...
    // Reverse it so the oldest is first, newest is last (chronological order for the AI)
    const dbHistory = rawHistory.reverse().map(msg => ({
      role: msg.role,
      content: msg.content
    }));
    const messages = [
    ...
    if (!completion) {
      ...
      if ((user.systemKeyUsage || 0) < 10) { // Increased limit slightly for testing
        try {
    ...
    const processedIntents = result.intents ? result.intents.map(intent => ({
      ...intent,
      email: user.email
    })) : [];
